   ...
   const lookups = [];
    ...
    try {
        ...
        if (isCSController) {
            ...
            // Process response data and merge combo lookups
           const processedData = await processResponseData({ responseData: response, model, dateColumns });
           setData({...processedData, lookups: transformComboRecords(response.combos || {})});
        } else {
            response = await transport(params);
            if (response.status === HTTP_STATUS_CODES.OK) {
                // Process response data and merge combo lookups
                const processedData = await processResponseData({ responseData: response.data, model, dateColumns });
