...
| `actions`              | `'grid' \| 'form' \| 'both'` | Controls where row actions (Copy, Delete) appear. `'grid'` keeps the pinned action column on the grid only (unchanged behavior). `'form'` hides the grid's action column and instead shows Copy and Delete as buttons in the edit Form's header, next to Save and Cancel, separated by a divider. `'both'` shows the action column on the grid **and** the buttons on the Form. Copy requires `permissions.copy: true` and is hidden for new (unsaved) records; Delete follows the Form's own permission check and is likewise hidden for new records. | `'grid'`           | No           |
