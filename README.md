...
| `localSortAndFilter`   | `boolean`           | When `true`, forces `paginationMode` to `client` and handles sorting/filtering entirely on the client side. The API is called once to load all data; subsequent sort, filter, and page interactions operate on the already-fetched rows without triggering additional API calls. | `false`           | No           |
| `staticData`           | `array \| object`   | Supplies static grid data instead of fetching from API. You can pass an array of rows, or `{ records, lookups, recordCount }`. Grid uses static mode only for these valid shapes; otherwise it falls back to API mode. In static mode, Grid uses client-side sorting/filtering/pagination. Export uses built-in client export for **CSV** and **Excel**. Other export formats are hidden in static mode and require the normal API/export endpoint flow.                                                                                                                                                                                                                                                             | -                 | No           |
...
| `exportTemplate`       | `string`            | Template configuration name passed with pivot export requests while preserving the normal export payload. Templates are stored inside `emailTemplates` at the project root. Example: `exportTemplate: "alert-notification"`.                                                                                                                                                                                                                                                  | -                 | No           |
| `configFileName`       | `string`            | Optional config file name passed with pivot export requests while preserving the normal export payload.                                                                                                                                                                                                                                                                                                                                                                           | -                 | No           |
| `pivotApi`             | `string`            | API endpoint used for pivot export requests when the **Excel With Pivot** option is selected. Other exports continue using the normal grid `api`.                                                                                                                                                                                                                                                                                                                               | -                 | No           |
...
| `actions`              | `'grid' \| 'form' \| 'both'` | Controls where the Copy and Delete row actions appear. `'grid'` keeps them in the pinned action column on the grid only (unchanged behavior). `'form'` moves Copy and Delete out of the grid's action column and shows them as buttons in the edit Form's header instead, next to Save and Cancel, separated by a divider; Edit is also dropped from the column since clicking the row already edits it. Any History/Download/custom actions stay in the grid's action column since they have no Form-header equivalent (the column itself is only omitted if none of those remain). `'both'` shows Copy and Delete in the grid's action column **and** as buttons on the Form. Copy requires `permissions.copy: true` and is hidden for new (unsaved) records; Delete follows the Form's own permission check and is likewise hidden for new records. | `'grid'`           | No           |
...
| `type`             | `string`                   | Specifies the type of data in the column (e.g., `number`, `string`). [Detail View](#field-components-properties-table)                                                                                                              | `string`        | `No`       |
...
| `localize`            | `boolean`                  | Controls timezone conversion for date/dateTime fields: `true` displays the value in the user's local timezone while storing/submitting it in UTC; `false` keeps the value in UTC for both display and storage/submission (no local-time conversion). | `false`         | `No`       |
...
To render the same tab-mapped fields as grouped sections (without stepper navigation), set `formConfig.showTabbed: "group"` and keep `tabs`/`tab` mappings the same.
`showGrouped: true` remains supported for backward compatibility.
If both grouped and tabbed options are provided, `showTabbed` takes precedence (`true` = stepper, `"group"` = grouped).
...
- `treeData`/`getTreeDataPath` stay wired for the grid's whole lifetime once `isServerGrouping` is `true` — never conditioned on whether a group field is currently selected. MUI does not support swapping row-grouping strategies (tree-data vs. column-based grouping) on a live grid; `getTreeDataPath` itself already degrades to `[String(row[idProperty])]` (a flat, single-level path) for every row once `rowGroupingField` is cleared, so the grid just renders flat instead of tearing down and rebuilding its internal row-tree strategy.
- MUI's auto-generated tree/group column would otherwise stay visible (empty) even while there's no active group field. While there's no active group field, it's hidden (`apiRef.current.setColumnVisibility`) and left out of the "manage columns" panel (`slotProps.columnsManagement.getTogglableColumns`) — both standard DataGridPremium APIs — so it only appears once a group field is actually selected.
