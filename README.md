...
| `showActionsInFormHeader` | `boolean`        | When `true`, hides the grid's pinned action column and instead shows Copy and Delete as buttons in the edit Form's header, next to Save and Cancel, separated by a divider. Copy requires `permissions.copy: true` on the Form and is hidden for new (unsaved) records; Delete follows the Form's existing `permissions.delete` check. | `false`           | No           |
