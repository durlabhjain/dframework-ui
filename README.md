...
| `localSortAndFilter`   | `boolean`           | When `true`, sorting and filtering are handled entirely on the client side by the DataGrid, regardless of the `paginationMode`. The API is called once (without sort/filter parameters) and all subsequent sort/filter interactions operate on the already-fetched data without triggering new API calls. Useful when all data is loaded in a single API request and you want the UI to handle sorting/filtering locally. | `false`           | No           |
